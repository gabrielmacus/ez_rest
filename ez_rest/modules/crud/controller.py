...
class BaseController[TModel: BaseModel]():
    ...
    @abstractmethod
    def update_by_id[TDtoIn:BaseDTO, TDtoOut:BaseDTO]( self, 
                    id:int,
                    partial_item:dict,
                    type_model:Type[TModel],
                    type_in:Type[TDtoIn],
                    type_out:Type[TDtoOut]) -> TDtoOut:
        item = self._repository.readById(id)
        if item is None:
        ...
        updated_item = self._repository.updateById(partial_data, id)

        return self._mapper_services.map(updated_item,
                                  type_out)
